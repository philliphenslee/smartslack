...
function SmartSlack(options) {
    ...
    // API references
    this.channels = require('./slack/channels');
    this.chat = require('./slack/chat');
    this.groups = require('./slack/groups');
    this.im = require('./slack/im');
    this.reactions = require('./slack/reactions');
    this.test = require('./slack/test');
    this.users = require('./slack/users');
...
SmartSlack.prototype.login = function () {
    ...
    rtm.start(function (err, results) {
        ...
        if (!results.ok) {
            _this.log.error(errors.slack_api_error + results.error);
            throw new Error(errors.slack_api_error + results.error);
        }
...
SmartSlack.prototype._onRtmEvent = function (message) {
    ...
    switch (message.type) {
        ...
        case 'team_join event':
            ...
            this.users.getList(function (err, results) {
                if (err) {
                    console.error(err);
                } else {
...
SmartSlack.prototype._sendToType = function (slackType, typeName, text, callback) {
    ...
    switch (slackType) {
        case slackTypes.CHANNEL:
            this.channels.get(typeName, function (err, result) {
                if (!err) {
                    entityId = result.id;
                }
            });
        ...
        case slackTypes.GROUP:
            this.groups.get(typeName, function (err, result) {
                if (!err) {
                    entityId = result.id;
                }
            });
