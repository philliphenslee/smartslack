...
function SmartSlack(options) {
    ...
    // Session data...
    global.session.user = null;
    global.session.users = null;
    global.session.channels = null;
    global.session.groups = null;
    global.session.ims = null;
    global.session.team = null;
    ...
    this.slack.rtm.start(function (err, results) {
        ...
        if (!err) {
            if (!results.ok) {
                ...
                global.session.user = results.self;
                global.session.users = results.users;
                global.session.channels = results.channels;
                global.session.groups = results.groups;
                global.session.ims = results.ims;
                global.session.team = results.team;
...
SmartSlack.prototype._sendToType = function (slackType, typeName, text, callback) {
    ...
    switch (slackType) {
        case slackTypes.CHANNEL:
            this.slack.channels.getChannel(typeName, function (err, result) {
                if (!err) {
                    entityId = result.id;
                }
            });
        ...
        case slackTypes.GROUP:
            this.slack.groups.getGroup(typeName, function (err, result) {
                if (!err) {
                    entityId = result.id;
                }
            });
